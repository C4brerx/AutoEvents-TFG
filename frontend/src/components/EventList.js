...
function EventList({ usuario }) {
    ...
    const [nuevoEvento, setNuevoEvento] = useState({
        titulo: '', descripcion: '', fecha: null, ubicacion: '', tipo: 'Concentración', aforo_maximo: ''
    });
    ...
    const manejarCrearOEditarEvento = async (e) => {
        ...
        formData.append('fecha', formatearFechaParaSQL(nuevoEvento.fecha));
        formData.append('ubicacion', nuevoEvento.ubicacion);
        formData.append('tipo', nuevoEvento.tipo);
        formData.append('aforo_maximo', nuevoEvento.aforo_maximo || '');
        ...
        if (eventoEnEdicion) {
            formData.append('id', eventoEnEdicion);
            formData.append('accion', 'editar_evento');
        } else {
            formData.append('accion', 'crear_evento');
        }
        ...
        try {
            ...
            if (datos.estado === 'exito') {
                ...
                setNuevoEvento({ titulo: '', descripcion: '', fecha: null, ubicacion: '', tipo: 'Concentración', aforo_maximo: '' });
                setArchivoFoto(null);
    ...
    return (
        <div>
            ...
            {/* RECOMENDACIÓN IA (DISEÑO VIP ACTUALIZADO) */}
            {pestañaActiva !== 'crear' && (
                <div className="mb-4 fade-in">
                    {!recomendacionIA && !cargandoIA && (
                        <button className="btn btn-outline-warning fw-bold rounded-pill shadow-sm border border-warning" onClick={pedirRecomendacionIA}>
                            <i className="bi bi-stars me-2"></i> IA: ¿A qué evento debería ir?
                        </button>
                    )}
                    ...
                    {recomendacionIA && !cargandoIA && (
                        <div className="alert bg-black bg-opacity-75 border border-warning rounded-4 shadow-lg position-relative p-4 fade-in">
                            <button
                                className="btn btn-sm btn-outline-warning border-0 position-absolute top-0 end-0 m-2 rounded-circle"
                                onClick={() => setRecomendacionIA('')}
                            >
                                <i className="bi bi-x-lg"></i>
                            </button>

                            <div className="d-flex align-items-center mb-3 pb-2 border-bottom border-warning border-opacity-25">
                                <div className="bg-warning text-dark rounded-circle d-flex justify-content-center align-items-center shadow me-3" style={{ width: '40px', height: '40px' }}>
                                    <i className="bi bi-stars fs-5"></i>
                                </div>
                                <div>
                                    <h5 className="text-warning fw-bold m-0">AutoEvents VIP</h5>
                                    <small className="text-white-50">Tu asesor de motor personal</small>
                                </div>
                            </div>

                            <p
                                className="text-light m-0"
                                style={{ fontSize: '1.05rem', lineHeight: '1.7', letterSpacing: '0.3px' }}
                                dangerouslySetInnerHTML={{
                                    __html: recomendacionIA.replace(/\*/g, '').replace(/\n/g, '<br/>')
                                }}
                            ></p>
                        </div>
                    )}
                ...
                <div className="card glass-card border-0 p-5 fade-in shadow-lg">
                    ...
                    <form onSubmit={manejarCrearOEditarEvento} className="row g-3">
                        <div className="col-md-8">
                            <label className="text-light">Título del Evento *</label>
                            <input type="text" className="form-control ae-input" required value={nuevoEvento.titulo} onChange={e => setNuevoEvento({...nuevoEvento, titulo: e.target.value})} />
                        </div>
                        <div className="col-md-4">
                            <label className="text-light">Tipo *</label>
                            <select className="form-select ae-input" value={nuevoEvento.tipo} onChange={e => setNuevoEvento({...nuevoEvento, tipo: e.target.value})}>
                        ...
                        <div className="col-md-6">
                            <label className="text-light">Ubicación *</label>
                            <input type="text" className="form-control ae-input" placeholder="Ej: Circuito de Jerez" required value={nuevoEvento.ubicacion} onChange={e => setNuevoEvento({...nuevoEvento, ubicacion: e.target.value})} />
                        </div>
                        <div className="col-12">
                            <label className="text-light">Descripción Corta</label>
                            <textarea className="form-control ae-input" rows="2" required value={nuevoEvento.descripcion} onChange={e => setNuevoEvento({...nuevoEvento, descripcion: e.target.value})}></textarea>
                        </div>

                        <div className="col-md-4">
                            <label className="text-light">Aforo Máximo</label>
                            <input type="number" className="form-control ae-input" placeholder="Opcional" value={nuevoEvento.aforo_maximo} onChange={e => setNuevoEvento({...nuevoEvento, aforo_maximo: e.target.value})} />
                        </div>

                        <div className="col-md-8">
                            <label className="text-light">Subir Cartel / Foto (Opcional)</label>
                            <input
                                type="file"
                                className="form-control ae-input"
                                accept="image/png, image/jpeg, image/webp"
                                onChange={e => setArchivoFoto(e.target.files[0])}
                            />
                            {eventoEnEdicion && <small className="text-secondary mt-1 d-block"><i className="bi bi-info-circle me-1"></i>Si no seleccionas nada, se mantendrá la foto actual.</small>}
                        </div>

                        <div className="col-12 text-end mt-4">
                            <button type="submit" className={`btn ${eventoEnEdicion ? 'btn-warning text-dark' : 'ae-btn-primary'} fw-bold px-5 py-2`} disabled={creando}>
                                {creando ? 'Guardando...' : (eventoEnEdicion ? 'Guardar Cambios' : 'Enviar Solicitud')}
                            </button>
                        </div>
                    </form>
                </div>
                                        ...
                                        >
                                            ...
                                            {usuario?.rol === 'admin' && (
                                                <div className="position-absolute top-0 start-0 m-2 d-flex gap-2" style={{ zIndex: 10 }}>
                                                    ...
                                                    <button className="btn btn-sm btn-warning shadow" onClick={(e) => {
                                                        e.stopPropagation();
                                                        setArchivoFoto(null);
                                                        setNuevoEvento({ titulo: evento.titulo, descripcion: evento.descripcion, fecha: new Date(evento.fecha), ubicacion: evento.ubicacion, tipo: evento.tipo, aforo_maximo: evento.aforo_maximo || '' });
                                            ...
                                            <div className="position-relative">
                                                <img
                                                    ...
                                                    alt={evento.titulo}
                                                    style={{ height: '220px' }}
                                                    onError={(e) => { e.target.onerror = null; e.target.src = 'https://images.pexels.com/photos/3802510/pexels-photo-3802510.jpeg?auto=compress&cs=tinysrgb&w=600'; }}
                                                />
